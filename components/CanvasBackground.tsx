...
const COLORS = [
  new THREE.Color('#0A0F2E'), // Blue Razz
  new THREE.Color('#1A0A00'), // Mango
  new THREE.Color('#0F0A1A'), // Grape
]
...
const fragmentShader = `
  ...
  void main() {
    vec2 uv = vUv;
    float noise = cnoise(vec3(uv * 2.5, uTime * 0.12));
    float noise2 = cnoise(vec3(uv * 1.2 + 0.5, uTime * 0.07 + 10.0));
    float combined = noise * 0.5 + noise2 * 0.5;
    vec3 color = uColor + combined * 0.06;
    // Radial vignette
...
export default function CanvasBackground() {
  ...
  useEffect(() => {
    ...
    const getTargetColor = () => {
      ...
      c.lerpColors(COLORS[lo], COLORS[hi], t)
      return c
    }
    ...
    const animate = () => {
      ...
      targetColor.copy(getTargetColor())
